...
class FunctionsV3
{
    ...
    {    
    	...
    	switch ($service) {
    		case 1:
    			$htm.='<li>'.t("Delivery").' <i class="icon_check_alt2 ok"></i></li></li>';
    			$htm.='<li>'.t("Take away").' <i class="icon_check_alt2 ok"></i></li></i></li>';
    			break;    			
    	
    		case 2:
    			$htm.='<li>'.t("Delivery").' <i class="icon_check_alt2 ok"></i></li></li>';
    			$htm.='<li>'.t("Take away").' <i class="icon_check_alt2 no"></i></li></i></li>';    			  				
    			break; 
    		case 3:
    			$htm.='<li>'.t("Delivery").' <i class="icon_check_alt2 no"></i></li></li>';
    			$htm.='<li>'.t("Take away").' <i class="icon_check_alt2 ok"></i></li></i></li>';
    			break; 
    ...
    {        
		...
		$sort_combine=$sort_by;
			...
			if ($getdata['sort_filter'] != ''){
				$sort="ASC";
				if($getdata['sort_filter']== 1){
					$sort="DESC";
				}
				$sort_combine=" ORDER BY ratings $sort";
			}
		...
		switch ($stype) {
			...
			case "kr_search_category":   				
			   $query=" cuisine REGEXP '(".$_GET['filter_cuisine'].")' ";
			  
			   $filter_ratings='';
				if (isset($_GET['ratings'])){		
					$filter_ratings=!empty($_GET['ratings'])?explode(",",$_GET['ratings']):false;
					if (is_array($filter_ratings) && count($filter_ratings)>=1){
						$x=1;
						foreach ($filter_ratings as $val) {				
							if (!empty($val)){
								if ( $x==1){
									$filter_rating.=" LIKE '%$val%'";
								} else $filter_rating.=" OR ratings LIKE '%$val%'";
								$x++;
							}
						}				
						if (!empty($filter_rating)){
						   $and.=" AND (ratings $filter_rating) ";
						}
					}				
    ...
    {    	
    	...
    	return Yii::app()->createUrl('store/searcharea?'.$new_request);    	
    }
    	...
    	if($merchant_info=FunctionsV3::getMerchantById($merchant_id)){
			...
			if (is_numeric($merchant_delivery_distance)){
				if ( $distance>$merchant_delivery_distance){
					 if($distance_type_raw=="ft" || $distance_type_raw=="millimeter" || $distance_type_raw=="mt"){
					 	return true;
					 }
		             return false;
                } else {
