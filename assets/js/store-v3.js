...
jQuery(document).ready(function() {		
	...
	$( document ).on( "click", ".back-map-address", function() {
		...
		/* cuisine_rateds */
	var Rate_stars  =[];
$('.Rate_stars').on('ifChanged ifUnchecked', function(event){
	dump(event.type);	
	var ratevalue = $(this).attr('id').split('_');
	if(event.type == 'ifChanged')
	{		
		Rate_stars.push(ratevalue[1]);		   		
	}
	if(event.type == 'ifUnchecked')
	{	
		var removeItem = ratevalue[1];	
		Rate_stars = $.grep(Rate_stars, function(value) {
			return value != removeItem;
		});
	}
	 	dump(Rate_stars);	 
	 
	var data1 = 'ratings=' + Rate_stars;
		all_filter(data1);
  });
	/* cuisine_Types */
	var cuisineIDarray  =[];
	$('#chkall').on('ifChanged ifUnchecked', function(event){
		$('.cuisine_check').each(function(){
				var cuisineID = $(this).attr('id').split('_');
				cuisineIDarray.push(cuisineID[2]);			
		});	
			if(event.type == 'ifChanged')
			{		
			
				$(' #cuisine_filterul .icheckbox_minimal').addClass('checked');	   		
			}
			if(event.type == 'ifUnchecked')
			{	
				$(' #cuisine_filterul .icheckbox_minimal').removeClass('checked');
				cuisineIDarray  =[];
			}
			var cuisineIDarray1 = cuisineIDarray;
			var cuisineIDval =  cuisineIDarray1.toString().replace(/\,/g,'|');
			//~ dump(cuisineIDval); 
			var data1 = 'filter_cuisine=' + cuisineIDval;
			all_filter(data1);
	});
	
$('.cuisine_check').on('ifChanged ifUnchecked', function(event){
	dump(event.type);	
	var cuisineID = $(this).attr('id').split('_');
	if(event.type == 'ifChanged')
	{		
	...
	if(event.type == 'ifUnchecked')
	{	
		var removeItem = cuisineID[2];	
		cuisineIDarray = $.grep(cuisineIDarray, function(value) {
			return value != removeItem;
		});
	}
	 	dump(cuisineIDarray);	
...
$('#sort_rating').on('change', function(){
	var sort_rating = $(this).val();	
	var data1 = 'sort_rating=' + sort_rating;
		all_filter(data1);
})
	
}); /*ready*/

function all_filter(dataval)
{
	$.ajax({ 
			  type: 'GET', 
			  url: window.location.protocol + '//' + window.location.host+sites_url+'/store/cuisine',
			  data: dataval,
			  success: function(msg){ 
					  dump("Sucess"+msg);
					return  $('#FilTer_result').html(msg);
					 cuisineIDarray  =[];
					   },
			  error: function(err){   
					  alert("failure"+err); } 
